...
defaults = {
    ...
    "pmi": 0.005, "use_chi_tax": True,
    # Closing Costs Detail
    "detail_closing": False, "cost_title": 2500, "cost_atty": 750, "cost_insp": 500, "cost_lender": 1000, "closing_costs_fixed": 3000,
    # Market
    "rent": 2800, "rent_inf": 0.03, "apprec": 0.03, "hold": 7, "inv_ret": 0.07,
    # Income/Debt
    "income": 150000, "status": "Single", "debt": 500, "roommate_income": 0,
    # Refinance
    "enable_refi": False, "refi_year": 3, "refi_rate": 0.055, "refi_cost": 3000,
    # Stress
...
with st.sidebar:
    ...
    with st.expander("🏠 Loan & Property", expanded=True):
        ...
        st.markdown("---")
        st.session_state.detail_closing = st.checkbox("Detailed Closing Costs", value=st.session_state.detail_closing)
        
        if st.session_state.detail_closing:
            st.session_state.cost_title = st.number_input("Title Fees", value=st.session_state.cost_title)
            st.session_state.cost_atty = st.number_input("Attorney", value=st.session_state.cost_atty)
            st.session_state.cost_insp = st.number_input("Inspection", value=st.session_state.cost_insp)
            st.session_state.cost_lender = st.number_input("Lender Fees", value=st.session_state.cost_lender)
            st.checkbox("Chicago Transfer Tax (0.75%)", key="use_chi_tax")
        else:
            st.session_state.closing_costs_fixed = st.number_input("Est. Closing Costs ($)", value=st.session_state.closing_costs_fixed)
            st.checkbox("Chicago Transfer Tax (0.75%)", key="use_chi_tax")

    with st.expander("🔮 Refinance Simulator"):
        st.session_state.enable_refi = st.checkbox("Simulate Future Refinance")
        if st.session_state.enable_refi:
            c1, c2 = st.columns(2)
            st.session_state.refi_year = c1.number_input("Refi Year", 1, 30, st.session_state.refi_year)
            st.session_state.refi_rate = c2.number_input("New Rate %", 0.0, 10.0, st.session_state.refi_rate*100, step=0.125) / 100
            st.session_state.refi_cost = st.number_input("Refi Cost ($)", value=st.session_state.refi_cost)
    ...
    with st.expander("👤 Income & House Hack"):
        st.session_state.income = st.number_input("Annual Income ($)", value=st.session_state.income, step=5000)
        st.session_state.roommate_income = st.number_input("Roommate/Airbnb Income ($/mo)", value=st.session_state.roommate_income)
...
def run_simulation(years, appreciation):
    ...
    if st.session_state.detail_closing:
        misc_closing = st.session_state.cost_title + st.session_state.cost_atty + st.session_state.cost_insp + st.session_state.cost_lender
    else:
        misc_closing = st.session_state.closing_costs_fixed
        
    initial_cash = DP + misc_closing + chi_tax
    ...
    curr_roommate = st.session_state.roommate_income
    loan_bal = L
    ...
    for y in range(1, years + 1):
        # Capture Start Balance for this year
        start_bal = loan_bal
        
        # --- REFINANCE LOGIC ---
        refi_cost_yr = 0
        if st.session_state.enable_refi and y == st.session_state.refi_year:
            new_r = st.session_state.refi_rate / 12
            new_n = 360 
            current_pi = loan_bal * (new_r * (1 + new_r)**new_n) / ((1 + new_r)**new_n - 1)
            refi_cost_yr = st.session_state.refi_cost
            r_mo = new_r 
        ...
        for _ in range(12):
            i = loan_bal * r_mo
            loan_bal -= (current_pi - i)
            int_yr += i
            
        principal_yr = (current_pi * 12) - int_yr
        ...
        if y == 1: y1_monthly_tax_savings = benefit / 12

        # Roommate
        roommate_annual = curr_roommate * 12
        
        # Costs
        cost_buy = (current_pi*12) + hoa_yr + tax + maint + ins + pmi + assess + refi_cost_yr - benefit - roommate_annual
        ...
        # Sunk Costs
        sunk_buy = int_yr + tax + hoa_yr + maint + ins + pmi + refi_cost_yr - benefit - roommate_annual
        
        data.append({
            "Year": y, 
            "Equity": curr_val - loan_bal, 
            "Renter Wealth": renter_wealth,
            "Buy Sunk": sunk_buy, 
            "Rent Sunk": cost_rent,
            "Tax Savings": benefit,
            "Interest Paid": int_yr,
            "Principal Paid": principal_yr,
            "Start Balance": start_bal,
            "End Balance": loan_bal
        })
        
        curr_val *= (1 + appreciation)
        curr_rent *= (1 + st.session_state.rent_inf)
        curr_hoa *= 1.04 
        curr_roommate *= (1 + st.session_state.rent_inf)
    ...
    # Year 1 Gross Housing (Before Tax/Roommate) for DTI
...
# TABS
tab1, tab2, tab3, tab4 = st.tabs(["📊 Snapshot", "⚠️ Sensitivity", "🏦 Loan Details", "📉 Sunk Costs"])
...
with tab1:
    ...
    with c4:
        net_cost = gross_housing - tax_savings - st.session_state.roommate_income
        sub_text = "After Tax & Roommate" if st.session_state.roommate_income > 0 else "After Tax Savings"
        card("Net Monthly", f"${net_cost:,.0f}", sub_text, "text-green")
...
with tab3:
    ...
    st.caption("Visualize the tipping point where you pay more Principal than Interest.")
    
    df_loan = pd.DataFrame(data).set_index("Year")
    
    # Chart
    st.bar_chart(df_loan[["Interest Paid", "Principal Paid"]], color=["#FF453A", "#30D158"])
    
    # Data Table
    st.dataframe(
        df_loan[["Start Balance", "Interest Paid", "Principal Paid", "End Balance"]].style.format("${:,.0f}"),
        use_container_width=True
    )

with tab4:
    st.subheader("Sunk Cost Analysis")
    st.write("Comparison of unrecoverable costs (Interest, Taxes, HOA vs Rent).")
